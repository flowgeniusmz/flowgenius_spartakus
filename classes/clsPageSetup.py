...
class PageSetup:
    ...
    def _initialize_page_display(self):
        ...
        PageUtilities.display_background_page(type=self.backgroundtype, style=self.backgroundstyle)
...
class PageUtilities:
    ...
    def get_styled_container1(height: int=None, border: bool=False):
        styleouter = st.secrets.styles.style_container3
        styleinner = st.secrets.styles.style_container4
        outer = sc(key="outer", css_styles=styleouter)
        with outer:
            inner = sc(key="inner", css_styles=styleinner)
            with inner:
